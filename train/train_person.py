...
def main():
    ...
    lr_scheduler = LRSequential([
        LRScheduler('linear', base_lr=0, target_lr=opt.lr,
                    nepochs=opt.warmup_epochs, iters_per_epoch=num_batches),  # warm up lr
    ...
    pretrained_net = get_model(model_name, pretrained=True, classes=1000, ctx=context)
    ...
    def train(ctx):
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            logger.info('[Epoch %d] validation: err-top1=%f top1=%f'%(epoch, err_top1_val, 1 - err_top1_val))

            if err_top1_val < best_val_score:
                best_val_score = err_top1_val
                net.save_parameters('%s/%.4f-myperson-%s-%d-best.params'%(save_dir, 1 - best_val_score, model_name, epoch))
                trainer.save_states('%s/%.4f-myperson-%s-%d-best.states'%(save_dir, 1 - best_val_score, model_name, epoch))
        ...
        if save_frequency and save_dir:
            net.save_parameters('%s/myperson-%s-%d.params'%(save_dir, model_name, opt.num_epochs-1))
            trainer.save_states('%s/myperson-%s-%d.states'%(save_dir, model_name, opt.num_epochs-1))
    ...
    if opt.imagenet_mode == 0:
        ...
        print('validation: err-top1=%f top1=%f' % (err_top1_val, 1 - err_top1_val))
    else:
        ...
        #export_block(param_save_path, net, preprocess=True, layout='CHW')
